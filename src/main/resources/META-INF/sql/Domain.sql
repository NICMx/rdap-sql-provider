#getByLdhName
SELECT dom_id, dom_handle, dom_ldh_name, dom_unicode_name, dom_port43, zone_id FROM {schema}.domain WHERE (dom_ldh_name=? OR dom_unicode_name=?) AND zone_id = ?;

#searchByPartialNameWZone
SELECT domain.dom_id, domain.dom_handle, domain.dom_ldh_name, domain.dom_unicode_name, domain.dom_port43, domain.zone_id FROM {schema}.domain WHERE (domain.dom_ldh_name LIKE ? OR domain.dom_unicode_name LIKE ? ) AND domain.zone_id = ? ORDER BY 1 LIMIT ?;

#searchByNameWZone
SELECT domain.dom_id, domain.dom_handle, domain.dom_ldh_name, domain.dom_unicode_name, domain.dom_port43, domain.zone_id FROM {schema}.domain WHERE (dom_ldh_name=? OR dom_unicode_name=?) AND domain.zone_id = ? ORDER BY 1 LIMIT ?;
...
SELECT d.dom_id, d.dom_handle, d.dom_ldh_name, d.dom_unicode_name, d.dom_port43, d.zone_id FROM {schema}.domain d WHERE d.zone_id IN (?)  AND (d.dom_ldh_name = ? OR d.dom_unicode_name = ?) ORDER BY 1 LIMIT ?;

#searchByPartialNameWOutZone
SELECT d.dom_id, d.dom_handle, d.dom_ldh_name, d.dom_unicode_name, d.dom_port43, d.zone_id FROM {schema}.domain d WHERE d.zone_id IN (?) AND (d.dom_ldh_name LIKE ? OR d.dom_unicode_name LIKE ?) ORDER BY 1 LIMIT ?;
