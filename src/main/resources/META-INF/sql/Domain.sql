...
#searchByNsIp
SELECT DISTINCT (dom.dom_id), dom.dom_handle, dom.dom_port43, dom.zone_id, dom.dom_unicode_name FROM {schema}.domain dom JOIN {schema}.domain_nameservers dom_ns ON dom_ns.dom_id = dom.dom_id JOIN {schema}.nameserver ns ON ns.nse_id = dom_ns.nse_id JOIN {schema}.ip_address ip	ON ip.nse_id = ns.nse_id WHERE ? > 0 AND (ip.iad_value = ? AND ip.iad_value = ?) ORDER BY 1 LIMIT ?;
